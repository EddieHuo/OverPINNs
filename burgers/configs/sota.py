...
def get_config():
    ...
    config.wandb.name = f"no_rwf_{datetime.datetime.now().strftime('%Y%m%d-%H%M%S')}"
    # wandb.name = "no_rwf"
    wandb.tag = None
    ...
    weighting.init_weights = ml_collections.ConfigDict({"ics": 1.0, "res": 1.0, "res2": 1.0})
    ...
    saving.save_every_steps = 500000
