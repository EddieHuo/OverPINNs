...
def evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    ckpt_path = os.path.join(
                os.path.abspath(workdir), eval_ckpt_path, 'ckpt'
            )
    model.state = restore_checkpoint(model.state, ckpt_path)
    params = model.state.params
    ...
    # 修改保存文件名
    l2_error_str = "{:.3e}".format(l2_error).replace('.', '')  # 格式化为科学计数法（例：1e-03）
    fig_path = os.path.join(save_dir, f"burger_l2error_{l2_error_str}.pdf")
