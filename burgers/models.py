...
class Burgers(ForwardIVP):
    ...
    def r_net(self, params, t, x):
        ...
        u_x = grad(self.u_net, argnums=2)(params, t, x)
        u_xx = grad(grad(self.u_net, argnums=2), argnums=2)(params, t, x)
        u_xxx = grad(grad(grad(self.u_net, argnums=2), argnums=2), argnums=2)(
            params, t, x
        )
        u_tx = grad(grad(self.u_net, argnums=1), argnums=2)(params, t, x)
        r1 = u_t + u * u_x - 0.01 / jnp.pi * u_xx
        r2 = u_tx + u_x **2 + u * u_xx - ( 0.01 / jnp.pi ) * u_xxx
        return r1
        # return r1 
    def r2_net(self, params, t, x):
        ...
        u_xxt = grad(grad(grad(self.u_net, argnums=2), argnums=2), argnums=1)(
            params, t, x
        )
        r1 = u_t + u * u_x - 0.01 / jnp.pi * u_xx
        r2 = u_tx + u_x **2 + u * u_xx - ( 0.01 / jnp.pi ) * u_xxx
        r3 = u_tt + u_t * u_x + u * u_xt - ( 0.01 / jnp.pi ) * u_xxt
        return r2
    ...
    def losses(self, params, batch):
        ...
        res2_pred = vmap(self.r2_net, (None, 0, 0))(params, batch[:, 0], batch[:, 1])
        res2_loss = jnp.mean((res2_pred) ** 2)

        loss_dict = {"ics": ics_loss, "res": res_loss, "res2": res2_loss}
        return loss_dict
